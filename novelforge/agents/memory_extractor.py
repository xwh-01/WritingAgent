...
_TITLE_SUFFIXES: set[str] = {
    "教练", "老师", "队长", "医生", "掌门", "长老", "师兄", "师姐",
    "师父", "师傅", "经理", "老板", "教授", "警官", "侦探", "将军",
    "统帅", "领主", "国王", "王子", "公主", "剑圣", "法师", "门主",
    "帮主", "会长", "局长", "司长", "总编", "导演", "船长", "舰长",
}
...
_CONSTRAINT_SIGNALS: list[tuple[str, str]] = [
    ...
    ("秘密|隐瞒|不能说|真相|揭开|藏|瞒|隐情", "Unresolved secret or concealed truth needs a reveal chapter."),
    ("伤|病|中毒|诅咒|虚弱|感染|发作|复发", "Track this condition/injury across later chapters until recovery is shown."),
    ("承诺|约定|发誓|誓言|保证|答应|必须还|欠", "Unfulfilled promise or oath — follow through in a later chapter."),
    ("威胁|追杀|通缉|敌人|仇|报复|埋伏|暗算", "Pending threat or enemy — resolve or escalate in a subsequent chapter."),
    ("分手|决裂|背叛|出卖|误会|翻脸|反目", "Relationship rupture — track whether it heals or deepens across chapters."),
    ("还差|未完成|尚未|还没|仍要|还要|仍需", "Unfinished task or goal — ensure it is addressed before being dropped."),
    ("新|变化|改变|突破|觉醒|升级|进阶|领悟", "Character growth or power change — keep consistent with this new baseline."),
]
...
class MemoryExtractorAgent(BaseAgent):
    ...
    def extract_chapter_memory(
        ...
        system = (
            "You are a long-novel memory extraction agent. Extract durable continuity facts "
            "from a chapter. "
    ...
    def _discover_chinese_characters(
        self, content: str, found: dict[str, Character]
    ) -> None:
        ...
        for match in re.finditer(rf"(?:^|[。！？，,\n\s])([{surname_alt}])([一-鿿]{{1,2}})(?:[，。！？、\n\s]|$|的|说|道|想|在|去|来|到|要|从|把|被|给|向|与|和|跟|对|让|就|都|也|却|还|便|却|将|已|正|刚|才|又|再)", content):
        ...
        for match in re.finditer(rf"([{''.join(_COMMON_SURNAMES)}]?\w{{1,3}}?)(?:{titles_alt})", content):
            raw = match.group(0)
            name = raw.rstrip("，。！？、\n ")
            character_id = self._slug(name)
    ...
    def _infer_trait_from_context(self, name: str, content: str) -> str:
        ...
        trait_signals = {
            "冷静": "沉着", "勇敢": "勇敢", "聪明": "聪明", "善良": "善良",
            "果断": "果断", "谨慎": "谨慎", "冲动": "冲动", "狡猾": "狡猾",
            "坚毅": "坚毅", "敏锐": "敏锐", "温柔": "温柔", "冷酷": "冷酷",
            "倔强": "倔强", "固执": "固执", "开朗": "开朗", "内敛": "内敛",
            "孤傲": "孤傲", "隐忍": "隐忍", "狂妄": "狂妄", "胆小": "胆小",
            "忠诚": "忠诚", "正直": "正直", "阴险": "阴险", "贪婪": "贪婪",
            "沉默": "沉默寡言", "幽默": "幽默", "乐观": "乐观", "悲观": "悲观",
        }
    ...
    def _detect_locations(
        ...
        for match in re.finditer(rf"(?:{location_alt})([一-鿿]{{2,6}})", content):
            ...
            if candidate in {"这里", "那里", "哪里", "外面", "里面", "前面", "后面", "上面", "下面"}:
                continue
            setting = WorldSetting(
    ...
    def _classify_relation(name_a: str, name_b: str, content: str) -> str:
        ...
        family_signals = {"父", "母", "兄", "弟", "姐", "妹", "儿", "女", "夫", "妻", "父女", "母子"}
