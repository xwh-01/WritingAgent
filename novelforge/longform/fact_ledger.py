...
from novelforge.core.models import CharacterFact, CharacterState, Story
...
class CharacterFactLedger:
            ...
            for state in sorted(states, key=lambda item: item.chapter):
                ...
                if state.emotional_state:
                    self._append_temporal(
                        generated, character_id, "emotional_state", state.emotional_state, state.chapter
                    )
                for knowledge in state.knowledge_gained:
                    self._append_persistent(generated, character_id, "knowledge", knowledge, state.chapter)
    ...
    def _append_persistent(
        ...
        if any(
            fact.character_id == character_id and fact.fact_type == fact_type and fact.value == value
            for fact in facts
        ):
            return
        facts.append(self._generated(character_id, fact_type, value, chapter))

    def _generated(self, character_id: str, fact_type: str, value: str, chapter: int) -> CharacterFact:
