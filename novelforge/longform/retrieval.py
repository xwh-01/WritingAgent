"""Explainable reranking for canonical knowledge and index hits."""
...
from novelforge.domain import RetrievalNote
...
class RankedKnowledge:
    """A retrieved item with its score and explainable ranking reasons."""
...
class RetrievalRanker:
    """Explainable reranker for vector hits and structured knowledge notes.

    权重优先级（从高到低）：
    - foreshadowing (6.0): 遗忘未回收伏笔 → 叙事断裂
    - character_state (6.5): 角色状态漂移 → 最常见的一致性错误
    - timeline_event (4.0): 因果链断裂 → 影响逻辑但出现频率低
    ...
    def __init__(self, config: "RetrievalConfig | None" = None) -> None:
        ...
        Args:
            config: RetrievalConfig 实例，为 None 时使用默认权重。
        """
    ...
    def rank_vector_hits(
        ...
        for hit in hits:
            ...
            overlap = query_terms.intersection(
                self._terms(document + " " + " ".join(str(v) for v in metadata.values()))
            )
            if overlap:
                score += min(self._query_match_max, len(overlap) * self._query_match_bonus_per_term)
                reasons.append("query")

            metadata_entities = set(str(metadata.get("entities", "")).split(",")) | {
            ...
            ranked.append(RankedKnowledge(hit, score, tuple(reasons)))
        ranked.sort(key=lambda item: item.score, reverse=True)
        return ranked[:limit]

    def rank_notes(
        self,
        cards: list[RetrievalNote],
        query: str,
        current_chapter: int,
        entities: set[str] | None = None,
        limit: int = 12,
    ) -> list[RankedKnowledge]:
        """对 RetrievalNote 列表进行多维度打分排序。
        ...
        for card in cards:
            ...
            overlap = query_terms.intersection(
                self._terms(card.content + " " + " ".join(card.tags))
            )
            if overlap:
                score += min(self._query_match_max, len(overlap) * self._query_match_bonus_per_term)
                reasons.append("query")
            ...
            ranked.append(RankedKnowledge(card, score, tuple(reasons)))
        ranked.sort(key=lambda item: item.score, reverse=True)
        return ranked[:limit]
