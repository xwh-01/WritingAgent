...
from novelforge.agents.knowledge_extractor import (
...
from novelforge.longform.knowledge_context import KnowledgeContextEngine
...
class StoryKnowledgeSystem:
    """Coordinate canonical long-form knowledge and context generation.

    为上层提供 process_new_chapter（处理新章节）和 build_writing_context（生成增强上下文）两个主要入口。
    """

    def __init__(
        self, llm: LLMClient | None = None, retrieval_config: "RetrievalConfig | None" = None
    ) -> None:
        """初始化所有子系统。
        ...
        self.knowledge_extractor = KnowledgeExtractorAgent(llm)
        self.summary_manager = SummaryManager(llm)
        self.context_engine = KnowledgeContextEngine(retrieval_config=retrieval_config)
    ...
    def process_new_chapter(self, story: Story, chapter_index: int, content: str) -> dict[str, Any]:
        ...
        返回包含各阶段结果的字典。
        """
        extraction = self.knowledge_extractor.extract_chapter_knowledge(
            story, chapter_index, content
        )
        self._apply_extraction(story, chapter_index, extraction)
        ...
        foreshadowings = self.foreshadowing_tracker.analyze_new_chapter(
            story, chapter_index, content
        )
        states = self.character_state_tracker.extract_state_from_chapter(
            story, chapter_index, content, list(story.design.characters.values())
        )
        ...
        history = [
            item
            for item in self.pacing_history.get(key, [])
            if item.get("chapter") != chapter_index
        ]
        ...
        knowledge = self.context_engine.process_chapter(
            story,
            chapter_index,
            summary,
            events,
    ...
    def _apply_extraction(
        ...
        """Record derived observations without mutating author-controlled design."""
        knowledge = story.knowledge
        knowledge.character_observations.extend(
            CharacterObservation(
        ...
        story.knowledge.chapter_constraints[chapter_index] = list(
            dict.fromkeys(extraction.continuity_constraints)
        )

    def build_writing_context(self, chapter_index: int, story: Story, query: str = "") -> str:
        ...
        pack = self.context_engine.build_context_pack(story, chapter_index, query=query)
        packed_context = self.context_engine.format_context_pack(pack)
        if packed_context.strip() != "NovelForge Writing Context":
        ...
        pending = [
            item
            for item in self.foreshadowing_tracker.get_pending(story)
        ...
        try:
            ...
            pass
        character_ids = set(story.design.characters)
    ...
    def review_chapter_consistency(
        ...
        for character_id, states in story.knowledge.character_states.items():
            ...
            if current:
                state_issues.extend(
                    self.character_state_tracker.check_consistency(previous, current)
                )
        return {
    ...
    def add_foreshadowing(
        ...
        item = Foreshadowing(
            id=f"fs-manual-{len(story.knowledge.foreshadowings) + 1}",
