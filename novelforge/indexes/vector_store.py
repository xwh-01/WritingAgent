...
from novelforge.indexes.interfaces import IVectorStore
...
class InMemoryVectorStore(IVectorStore):
    ...
    def add(
        self, collection: str, documents: list[str], metadatas: list[dict[str, Any]], ids: list[str]
    ) -> None:
    ...
    def query(
        ...
        for doc_id, (document, metadata) in self._collections.get(collection, {}).items():
            ...
            scored.append(
                {"id": doc_id, "document": document, "metadata": metadata, "score": score}
            )
    ...
    def delete_story(self, story_id: str) -> int:
        ...
        for docs in self._collections.values():
            ids = [
                ...
                if str(metadata.get("story_id", "")) == story_id
                or doc_id.startswith(f"{story_id}:")
            ]
            for doc_id in ids:
                docs.pop(doc_id, None)
                deleted += 1
    ...
    def query(
        ...
        if self.client is None:
            return self._fallback.query(
                collection, query_text, k, story_id=story_id, max_chapter=max_chapter
            )
        ...
        for collection in collections:
            ...
            try:
                ...
                ids.extend(
                    [
                        doc_id
                        for doc_id in all_items.get("ids", [])
                        if doc_id.startswith(f"{story_id}:")
                    ]
                )
            except Exception:
                pass
